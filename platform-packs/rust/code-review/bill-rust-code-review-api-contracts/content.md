...
- Require evolving public enums to use `#[non_exhaustive]` where downstream matching must remain extensible; reject a new variant that breaks exhaustive consumers or silently changes fallback behavior.
...
- Verify FFI layouts use an intentional `#[repr(C)]`, fixed-width fields, and explicit string encoding; reject ABI corruption from compiler-dependent layout or enum representation.
- Require every `extern "C"` ownership transfer to pair constructors and destructors and prevent unwinding; reject leaks, double frees, or cross-language crashes.
