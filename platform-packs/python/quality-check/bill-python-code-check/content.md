...
4. Run the pack's quality-check entrypoint through the existing locked invocation such as `uv run --frozen`, `poetry run`, `.venv/bin/python`, `pdm run`, `hatch run`, `tox`, or `nox`; do not install dependencies, create an environment, upgrade tools, or rewrite a lockfile merely to run checks.
