...
1. Consume the caller's exact resolved scope as the files in scope, including its staged, committed branch-diff, and untracked files, without recomputing it through a bare `git diff --name-only`; identify affected packages, applications, tests, migrations, workers, notebooks, generated contracts, and public interfaces before running commands.
...
10. Build and inspect distributions with the configured backend through `python -m build --no-isolation`, an owned frozen or offline equivalent, Hatch, PDM, Poetry, or the repository wrapper; never let build isolation provision dependencies or access the network. Use `twine check dist/*` or owned wheel-install smoke tests when configured, and verify generated artifacts rather than committing them.
