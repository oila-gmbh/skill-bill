...
Review Python product code, tests, packaging, and project-owned automation through this Python pack. In a repository with multiple strong platform signals, let other stack packs own non-Python product files. Do not route a frontend, Kotlin, PHP, iOS, or generated-client change through Python only because a Python helper, virtual environment marker, or CI script is nearby.
