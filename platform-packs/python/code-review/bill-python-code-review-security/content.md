...
- Reject `subprocess.run(..., shell=True)` and attacker-controlled executable, option, cwd, or environment values; shell expansion and option injection allow command execution.
...
- Require route, object, and tenant authorization before Django, SQLAlchemy, or service-layer fetches and mutations; ambient identity without resource checks enables privilege escalation and data exposure.
...
- When browser sessions are detected, verify Django or Flask CSRF middleware, `SameSite`, `Secure`, `HttpOnly`, trusted hosts, and proxy headers match deployment; weakened session boundaries enable forged or stolen requests.
