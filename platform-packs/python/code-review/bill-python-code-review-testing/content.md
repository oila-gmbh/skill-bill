...
- Require retry, idempotency, and worker tests to prove duplicate delivery, terminal failure, acknowledgement, and exactly-once effects where promised; one successful execution does not prevent data duplication.
- Require deterministic time via `freezegun`, an injected clock, or controlled event-loop time rather than `time.sleep`; timing luck causes flaky tests and masks deadline regressions.
