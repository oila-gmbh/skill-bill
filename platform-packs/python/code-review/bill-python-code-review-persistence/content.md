...
- Require rollback and bounded retry around classified serialization failures or deadlocks, and require each retried mutation to use a uniqueness constraint, durable deduplication record, idempotency key, or demonstrably idempotent operation; retrying arbitrary `IntegrityError` values can duplicate writes or mask invalid data.
