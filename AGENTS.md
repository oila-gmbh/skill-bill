...
- Before review, the runtime creates the active subtask commit or amends its proven owned HEAD, records the exact reviewed target/tree identities, and launches review only after durable identity persistence. Later code changes invalidate that approval; a message-only amend with the same tree may carry it forward. Finalisation stages only the declared boundary-history paths (`agent/history.md` and `agent/decisions.md` under boundary roots) after verifying the reviewed tree; every other post-review change re-enters audit and review. The runtime captures the final sha, pushes, and records `commit_sha` into the decomposition manifest. The agent supplies the outcome message in `commit_push_result` (`changed_paths` is advisory); it does not run `git commit` or `git push` for a subtask.
