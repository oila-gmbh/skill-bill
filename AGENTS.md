...
Validate-phase build, test, and gate execution is runtime-owned when the dominant platform pack declares `validation_gate`. The loop is run the gate, hand the agent every finding from that one run, repair, rerun to confirm — repeating only while findings remain, and blocking loudly once a repair stops converging. There is no per-fix gate run, no substantiation receipt or coverage gate, no finding paging, and no cache-bypassing confirmation pass. FULL depth runs the pack's collect-all argv; BUILD_ONLY intermediate validate runs the pack's build-only argv. The validate agent repairs findings and must not invoke the gate or any quality-check skill. When a pack declares no gate, validate falls back to agent-run behavior and that degradation is surfaced. Audit and repair evidence remain read-only repository facts — never builds or tests.
