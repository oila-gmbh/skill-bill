...
Validate-phase build, test, and gate execution is runtime-owned when the dominant platform pack declares `validation_gate`: last-subtask FULL validate collects all observable failures in one discovery run and proves repairs by confirmation identity closure. The runtime runs the pack-declared argv, measures each run, and hands the agent a bounded finding-set projection. The validate agent repairs findings and must not invoke the gate or any quality-check skill. BUILD_ONLY intermediate validate stays compile/build-only without FULL substantiation receipts or collect-all confirmation closure. When a pack declares no gate, validate falls back to agent-run behavior and that degradation is surfaced. Audit and repair evidence remain read-only repository facts — never builds or tests.
