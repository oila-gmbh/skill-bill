...
for this classification. The classification is parsed by the shared authoring
and install-plan discovery seams and enforced identically at `skill-bill
validate` time.

### Parent rules and loud-fail behavior

The authoring/install pipeline loud-fails with a typed, actionable error
(`InvalidInternalSkillClassificationError` at authoring; the same rules at
`RepoValidationRuntime` validate time) when an internal skill's parent is:

- **missing** — the `internal-for` value is empty or the key has no value;
- **unknown** — no listed skill in the repo matches the declared parent name;
- **itself internal** — chaining `internal-for` is not allowed; depth is 1; a
  parent that is itself internal is rejected so a sidecar cannot host another
  sidecar;
- **self** — a skill cannot declare itself as its own parent.

A separate sidecar-name collision guard (`InternalSkillSidecarCollisionError`)
fails when two internal skills under the same parent would render to the same
`<skill-name>.md` filename.
...
re-renders and idempotency holds.
