...
`kmp` covers Android and Kotlin Multiplatform, declaring platform-correctness, persistence, reliability, UI, and UX/accessibility on the required Kotlin baseline, and routes quality checks to `bill-kmp-code-check` (no Kotlin fallback). `bill-feature-verify` remains pre-shell.
