...
ALLOWED_PACKAGES = ("base", "kotlin", "kmp", "backend-kotlin", "go")
...
def validate_skill_location(skill_name: str, skill_file: Path, issues: list[str]) -> None:
  ...
  if package_name == "base":
    if any(skill_name.startswith(prefix) for prefix in ("bill-kotlin-", "bill-kmp-", "bill-backend-kotlin-", "bill-go-", "bill-gradle-")):
      issues.append(
