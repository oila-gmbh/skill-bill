...
Go, iOS, Kotlin, PHP, Python, Rust, and TypeScript each own all ten approved review areas. The KMP pack covers Android and Kotlin Multiplatform: it owns six declared areas — `architecture`, `platform-correctness`, `persistence`, `reliability`, `ui`, and `ux-accessibility` — and composes the remaining four (`performance`, `security`, `testing`, `api-contracts`) from its required Kotlin baseline. Every pack routes quality checks directly to its own manifest-declared checker, including KMP through `bill-kmp-code-check`.
...
- `kmp` — Android and Kotlin Multiplatform review, declaring `architecture`, `platform-correctness`, `persistence`, `reliability`, `ui`, and `ux-accessibility` on top of the required Kotlin baseline, plus governed Android add-ons and direct multiplatform quality-check behavior
