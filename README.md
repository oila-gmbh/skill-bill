...
| `/bill-feature-task` | Runtime-backed trigger that runs a governed spec through the `skill-bill feature-task` phase loop |
...
| `/bill-feature-task-legacy` | DEPRECATED prose orchestrator superseded by `bill-feature-task`; retained as a temporary fallback during the removal window |
