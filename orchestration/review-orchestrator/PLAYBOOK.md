...
- It also accepts at most one governed caller context, `context:feature-remediation`. This context is valid only with `mode:inline` for a bounded feature-task re-review of the complete run-owned delta from its durable immutable base. Reject it with any other mode or scope.
...
- `inline` is authoritative: run every routed baseline and signal-relevant rubric sequentially in the current context regardless of size or risk. Do not spawn specialists, invent lane totals, refuse the request, or silently change it to another mode.
