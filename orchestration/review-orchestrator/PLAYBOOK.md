...
- `context:feature-remediation` bounds a governed feature-task re-review to the exact checkpoint remediation delta. Like every explicit inline request, it runs every signal-relevant baseline and specialist rubric inline and reports high-risk signals as review coverage rather than refusing or delegating the pass. This context does not weaken finding severity, evidence, or approval rules.
