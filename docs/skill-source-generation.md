...
code-review family is the worked example: 67 review-pack skills — seven stack
entries plus their 60 specialists — all carry `internal-for: bill-code-review`
...
The code-review family is the platform-pack worked example. Exactly 67
review-pack skills are internal — every skill under
`platform-packs/{go,ios,kotlin,kmp,php,python,rust}/code-review/`: seven stack entry skills
(`bill-ios-code-review`, `bill-kotlin-code-review`, `bill-kmp-code-review`,
`bill-go-code-review`, `bill-php-code-review`, `bill-python-code-review`, `bill-rust-code-review`) plus their 60 area specialists. All 67 carry
`internal-for: bill-code-review` and install as siblings inside
`bill-code-review/`'s staged directory; the seven stack entries do **not** become
...
`bill-code-review-parallel` and `bill-code-check`) but none of the 67.
...
specialists) stage; the other 58 contribute nothing (PD3). The KMP pack declares
...
`bill-go-code-check`, `bill-kotlin-code-check`, `bill-php-code-check`, `bill-python-code-check`, `bill-rust-code-check`) carry
