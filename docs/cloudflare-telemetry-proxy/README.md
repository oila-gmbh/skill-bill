...
- Retired event names (`skillbill_feature_task_prose_*`, `skillbill_feature_implement_*`, `skillbill_goal_prose_*`) are silently ignored and dropped: the Worker never rejects them and no `/stats` aggregate counts them. Already-installed clients on older Skill Bill versions must keep receiving normal success responses rather than errors, so the drop is deliberate and unreported.
