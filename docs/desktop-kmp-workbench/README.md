# Desktop KMP Workbench Spec

Status: Draft

## Summary

Skill Bill should ship an optional desktop workbench for browsing, editing, validating, and publishing governed Skill Bill repo changes. The app is repo-based: the user opens a local `skill-bill` checkout, edits authored source through the existing Skill Bill runtime, validates the repo, then commits and pushes changes through Git.

The workbench must not become a second implementation of Skill Bill governance. It is a graphical client over the same authoring, scaffolding, validation, rendering, install, and manifest-discovery contracts that already power the CLI and MCP surfaces.
...
runtime-kotlin/
  ...
  runtime-desktop/
    ...
    core/
      ...
      designsystem/        # shared theme, colors, metrics, reusable UI primitives
    ...
    feature/
      workbench/           # screen component, view model, state, and UI
```
...
library, KMP Compose, KMP application, and kotlin-inject/KSP/Anvil wiring; shared
UI, domain, data, and feature code live in nested `:runtime-desktop:core:*` and
...
`:runtime-desktop` app module's `jvmMain`; JVM desktop tests live in the owning
feature/core modules. The first target is JVM desktop; Android and iOS app
targets remain out of scope until they are explicitly needed.

Recommended internal layers:

- `SkillBillDesktopApp`: Compose entrypoint and window lifecycle
- `WorkbenchViewModel`: screen state, selection, command orchestration
...
- opens a valid repo and renders the two-panel workbench
...
- [ ] Add README or getting-started link to the workbench.
