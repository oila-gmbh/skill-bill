...
`/bill-code-review` accepts `mode:auto|inline|delegated`: omission and `mode:auto` both resolve to `inline` for every pass and for a scope with no pass number; `mode:inline` runs the light depth tier in one review subagent covering the routed areas at reduced depth under a bounded budget, with no specialist fan-out and not equivalent coverage to delegated; and `mode:delegated` is the experimental full-depth tier reached only by explicit selection, launching one specialist subagent per routed area. Feature workflows expose the same choice as `code-review:auto|inline|delegated`.
...
- **Branch strategy is declared, not improvised**: defaults to `same_branch_commit_per_subtask` (one commit per subtask on the parent feature branch); `stacked_branches` is an explicit opt-in where the runtime refuses to advance if the current branch/base does not match the manifest. When every subtask is complete, goal finalization commits any remaining dirty work on the feature branch (`git add -A`) and pushes before opening or reusing the PR.
