...
| Level | What is sent |
|-------|-------------|
| `off` | Nothing is transmitted; `sync` and `autoSync` short-circuit on the disabled level. Three events are still queued locally: `skillbill_runtime_exception` (message redacted to `[redacted]`, `skillbill.` frames only), `skillbill_feature_task_runtime_projection_measurement` (bounded counters, the repository checkpoint fingerprint, `contract_version`, `consumer_phase_id`, and a raw `workflow_id`), and `skillbill_feature_task_runtime_shared_evidence` (checkpoint fingerprint, consuming phase id, outcome, and bounded index counters). Enabling telemetry later does not discard those rows — `clearsPendingOutbox("off", "anonymous")` is `false` — so they can upload on the next sync. |
