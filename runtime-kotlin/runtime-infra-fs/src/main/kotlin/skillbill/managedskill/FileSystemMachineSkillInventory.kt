...
class FileSystemMachineSkillInventory(
  ...
  override fun read(request: ReadMachineSkillInventoryRequest): MachineSkillInventorySnapshot {
    val store = FileManagedSkillRecordStore.openReadOnly(request.home)
    val recordDiscoveries = store?.listRecords().orEmpty()
    val records = recordDiscoveries.mapNotNull(ManagedSkillRecordDiscovery::record).associateBy(ManagedSkillRecord::name)
    ...
    val validGroups = nonProduct.groupBy { normalizeManagedSkillName(it.rawName) }
    validGroups[null].orEmpty().forEach { occurrence ->
      diagnostics += MachineSkillInventoryDiagnostic("INVALID_NAME", occurrence.path, "Invalid skill name '${occurrence.rawName}'.")
    }
    val rows = validGroups.filterKeys { it != null }.map { (name, group) ->
      aggregate(name!!, group, request.targets.map { it.id })
    ...
    store?.listSnapshotPaths().orEmpty().filter { it !in expectedSnapshots && !isVerifiedProductStaging(it) }.forEach { path ->
      diagnostics += MachineSkillInventoryDiagnostic("ORPHAN_SNAPSHOT", path, "Snapshot is not referenced by a managed record.")
    }
    val targetStates = request.targets.map { target ->
      val providerConflicts = request.targets.filter { it.id.skillsPath == target.id.skillsPath }.map { it.id.provider }.distinct()
      val issues = target.issues.map { MachineSkillIssue("DUPLICATE_TARGET", it) } +
        if (providerConflicts.size > 1) listOf(MachineSkillIssue("PROVIDER_PATH_CONFLICT", "Path is shared by multiple providers.")) else emptyList()
...
private fun scanTarget(
  ...
  return Files.newDirectoryStream(target.skillsPath).use { entries ->
    entries.map { path -> scanOccurrence(target, path, records[normalizeManagedSkillName(path.fileName.toString())], store, baselineNames) }
...
private fun scanOccurrence(
  ...
  val product = rawName == ".bill-shared" || rawName in baselineNames ||
    (resolved != null && isVerifiedProductStaging(resolved) && resolved.fileName.toString().startsWith("$rawName-"))
  val exactOwnedLink = expected != null && resolved == expected
  val exists = resolved?.let { Files.exists(it) } ?: true
  val linkHealth = when {
  ...
  val scanPath = resolved?.takeIf(Files::exists) ?: path
  val bundle = if (kind == MachineSkillEntryKind.OTHER || (resolved != null && !exists)) null else
    runCatching { OpaqueSkillBundleScanner().scan(scanPath, emptySet()) }.getOrNull()
  val issues = mutableListOf<MachineSkillIssue>()
  if (record != null && !Files.exists(record.sourcePath)) issues += MachineSkillIssue("MISSING_SOURCE", "Managed source is missing.", MachineSkillIssueSeverity.ERROR)
  if (record != null && bundle != null && exactOwnedLink && bundle.contentHash != record.activeContentHash) {
    issues += MachineSkillIssue("HASH_MISMATCH", "Installed content does not match the managed record.", MachineSkillIssueSeverity.ERROR)
  }
  ...
  return MachineSkillOccurrence(target, rawName, path.toAbsolutePath().normalize(), kind, ownership, linkHealth, bundle?.contentHash, setOfNotNull(if (record != null) "managed-record" else null, if (product) "product-evidence" else null), issues)
}
...
private fun aggregate(
  ...
  val health = when {
    issues.any { it.code == "HASH_MISMATCH" } -> MachineSkillHealth.HASH_MISMATCH
    occurrences.any { it.linkHealth == MachineSkillLinkHealth.EXPECTED_BROKEN || it.linkHealth == MachineSkillLinkHealth.BROKEN } -> MachineSkillHealth.BROKEN_LINK
  ...
  return MachineSkillInventoryRow(name, occurrences.minOf(MachineSkillOccurrence::rawName), ownership, health, presence, hashes, divergent, issues.distinct().sortedBy(MachineSkillIssue::code))
}
...
private fun isVerifiedProductStaging(path: Path): Boolean = Files.isDirectory(path, NOFOLLOW_LINKS) &&
  Files.isRegularFile(path.resolve(".content-hash"), NOFOLLOW_LINKS)
