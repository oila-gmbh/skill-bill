      ...
      DatabaseMigration(
        ...
        operation = { connection ->
          connection.createStatement().use { statement ->
            ...
            statement.execute("ALTER TABLE feature_task_convergence_records RENAME TO feature_task_convergence_records_v16")
            statement.execute(
              """
              CREATE TABLE feature_task_convergence_records (
                record_id TEXT PRIMARY KEY CHECK(length(record_id) BETWEEN 1 AND 160),
                contract_version TEXT NOT NULL CHECK(contract_version = '0.1'),
                workflow_id TEXT NOT NULL CHECK(length(workflow_id) BETWEEN 1 AND 160),
                record_kind TEXT NOT NULL CHECK(record_kind IN (
            ...
            statement.execute(
              "ALTER TABLE feature_task_convergence_legacy_imports RENAME TO feature_task_convergence_legacy_imports_v16",
            )
            statement.execute(
              """
              CREATE TABLE feature_task_convergence_legacy_imports (
                workflow_id TEXT NOT NULL,
                source_digest TEXT NOT NULL CHECK(length(source_digest) = 64),
                disposition TEXT NOT NULL CHECK(disposition IN ('imported', 'quarantined')),
                reason_code TEXT CHECK(reason_code IS NULL OR length(reason_code) BETWEEN 1 AND 64),
            ...
            statement.execute(
              "CREATE INDEX idx_convergence_history ON feature_task_convergence_records(workflow_id, generation, created_at)",
            )
            statement.execute(
              "CREATE INDEX idx_convergence_unresolved ON feature_task_convergence_records(workflow_id, record_kind, record_status, classification, generation)",
            )
          }
        },
      ),
