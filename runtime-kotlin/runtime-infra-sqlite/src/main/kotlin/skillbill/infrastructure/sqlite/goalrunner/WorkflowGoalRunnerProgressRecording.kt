...
internal class WorkflowGoalRunnerProgressRecording(
  ...
  fun progress(workflowId: String): GoalRunnerWorkflowProgress? = database.read { unitOfWork ->
    ...
    val record = family.get(unitOfWork.workflowStates, WorkflowId(workflowId)) ?: return@read null
    ...
    GoalRunnerWorkflowProgress(
      workflowId = WorkflowId(record.workflowId),
  ...
  fun recordObservabilityEvent(request: GoalRunnerObservabilityRecordRequest): Boolean =
    database.transaction { unitOfWork ->
      ...
      val record = family.get(unitOfWork.workflowStates, WorkflowId(request.workflowId))
        ?: return@transaction false
  ...
  fun progressEvents(workflowId: String): List<Map<String, Any?>> = database.transaction { unitOfWork ->
    ...
    val record = family.get(unitOfWork.workflowStates, WorkflowId(workflowId))
      ?: return@transaction emptyList()
  ...
  fun childWorkflowLoopIterations(workflowId: String): Map<String, Int> = database.read { unitOfWork ->
    ...
    val record = family.get(unitOfWork.workflowStates, WorkflowId(workflowId)) ?: return@read emptyMap()
    val artifacts = decodeArtifacts(record.artifactsJson)
  ...
  private fun appendHistoryArtifact(append: HistoryArtifactAppend): Boolean = database.transaction { unitOfWork ->
    val family = workflowFamilyFor(unitOfWork.workflowStates, append.workflowId.value)
      ?: return@transaction false
