...
The feature `spec.md` remains the governed product contract; it is not a mutable workflow ledger. A sibling `decomposition-manifest.yaml` is the sole prepared-feature authority marker and always contains one or more executable subtasks; a bare `spec.md` is preparation intake. Continuation lookup remains authoritative and precedes artifact discovery. Pre-planning, planning, phase outputs, and the phase ledger remain durable database artifacts. Initial implementation continuation is hydrated from the completed `plan`. Audit repair uses a durable cycle with diagnosis, authorized repair, checkpoint attachment, final audit, satisfied, and paused stages. The cycle keeps the original preplan and plan immutable, binds each launch and stage request to the workflow, execution, provider session, and worker fence, and records all criterion gaps and repair outcomes before the final audit. Provider adapters must return the continuation identity they actually used. The engine captures and verifies the post-repair checkpoint before acknowledging final-audit evidence, and a resumable checkpoint ref lets recovery attach a checkpoint created before interruption. Agents acknowledge stages through `feature_task_audit_stage`; the CLI `feature-task audit-stage` command is the adapter when MCP is unavailable. A failed or incomplete cycle pauses the audit rather than handing gaps to a separate implement phase. Review remediation is a single bounded round: `review` runs once, `changes_requested` may launch one `implement_fix` pass (`review_fix` cap 1, then advance to `validate`), and review does not run again after that fix.
