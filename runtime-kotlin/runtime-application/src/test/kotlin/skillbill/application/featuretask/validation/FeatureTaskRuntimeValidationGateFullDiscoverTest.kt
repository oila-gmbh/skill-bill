...
class FeatureTaskRuntimeValidationGateFullDiscoverTest {
  ...
  @Test
  fun `one check hands every finding to one repair and confirms with a second check`() {
    ...
    val runner = ScriptedGateRunner(listOf(failedWith(compiler, laterTest), passed()))
    ...
    assertTrue(runner.requests.all { it.argv == listOf("echo", "collect-all") })
    assertTrue(runner.requests.all { it.cacheMode == ValidationGateCacheMode.CACHE_ELIGIBLE })
    assertTrue(runner.requests.all { it.findingParseMode == ValidationGateFindingParseMode.COLLECT_ALL })
    assertTrue(runner.requests.none { it.argv == declaration.fullGateCommand })
    assertTrue(runner.requests.none { it.terminalVerifying })
  ...
  fun `a large finding set is never split across repair launches`() {
    ...
    val runner = ScriptedGateRunner(listOf(failedWith(*findings.toTypedArray()), passed()))
  ...
  fun `findings still present after a repair become the next repair set`() {
    ...
    val runner = ScriptedGateRunner(listOf(failedWith(discovery), failedWith(remaining), passed()))
    val cycle = coordinator(declaredResolver(), runner, mutableListOf()).execute(
      cycle = fullCycle { findings, _ ->
  ...
  fun `a repair that never converges blocks instead of looping forever`() {
    ...
    val runner = ScriptedGateRunner(List(20) { failedWith(finding) })
    val cycle = coordinator(declaredResolver(), runner, progress).execute(
