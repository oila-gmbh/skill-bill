...
class FeatureTaskRuntimePhasePromptComposerTest {
  ...
  fun `initial preplan prompt excludes review mode, commit-PR, and finalization mandate text`() {
    ...
    assertFalse(prompt.contains("produced_outputs"), "preplan must not ask for produced_outputs")
    assertFalse(prompt.contains("Required final output"), "preplan must not ask for a JSON output contract")
  ...
  fun `a prior settlement failure is surfaced as a corrective directive on retry`() {
    ...
    listOf("review", "audit").forEach { phaseId ->
      ...
      assertContains(retry, "Restate your phase outcome in plain prose", false, "$phaseId retry asks for prose")
      assertFalse(retry.contains("REJECTED by the schema gate"), "$phaseId retry must not resurrect schema gate")
    }
  }
  ...
  fun `audit remediation names the criteria it must implement in this invocation`() {
    ...
    assertContains(prompt, "Follow that plan completely")
    assertFalse(prompt.contains("repair_item_results"), "the remediation round owes no per-item results")
  }
...
private fun projectionExampleCases() = listOf(
  ...
  Triple(
    implementPhase,
    receiptKind,
    briefingFor(implementPhase, unmetCriterionRefs = listOf("AC-004: gap note")),
  ),
)
