...
class FeatureTaskRuntimePhasePromptComposerTest {
  ...
  fun `audit remediation output contract names every carried item and required evidence field`() {
    ...
    assertContains(prompt, "fix_verified when outcome is fixed")
    assertContains(prompt, "already_satisfied_verified when outcome is already_satisfied")
    assertContains(prompt, "recurrence_verified is for recurring audit-gap disposition evidence")
