...
class FeatureTaskRuntimeGoalContinuationStateRecorder(
  ...
  fun reviewState(workflowId: WorkflowId): GoalSubtaskReviewState? = database.read { unitOfWork ->
    val record = WorkflowFamily.TASK_RUNTIME.get(unitOfWork.workflowStates, workflowId) ?: return@read null
    reviewStateFromArtifacts(decodeArtifacts(record.artifactsJson))
  }

  fun continuation(workflowId: WorkflowId): FeatureTaskRuntimeGoalContinuationArtifact? = database.read { unitOfWork ->
    val record = WorkflowFamily.TASK_RUNTIME.get(unitOfWork.workflowStates, workflowId) ?: return@read null
