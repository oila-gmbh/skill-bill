...
class GoalPlanningPreparationValidator(
  ...
  fun canonicalize(record: GoalPlanningPreparationRecord): GoalPlanningPreparationRecord {
    ...
    val preplan = requireGoalPlanningPhaseEnvelope(record.preplanPayload, PREPLAN_PHASE_ID, label).envelope
    val plan = requireGoalPlanningPhaseEnvelope(record.planPayload, PLAN_PHASE_ID, label).envelope
