...
internal object GoalChildOrphanReplacementPrep {
  fun prepare(request: GoalChildOrphanReplacementRequest): OrphanPrep {
    ...
    val baselineCapture = request.reviewBaseline?.let {
      GoalSubtaskReviewBaselineResult(status = "ok", baseline = it)
    } ?: request.gitOperations.captureGoalSubtaskReviewBaseline(request.repoRoot, branch)
  ...
  private fun isOrphanCandidate(subtask: DecompositionSubtask): Boolean {
    return PortableReviewBaselineValidator.isSafePreImplementationRecovery(subtask)
  }
