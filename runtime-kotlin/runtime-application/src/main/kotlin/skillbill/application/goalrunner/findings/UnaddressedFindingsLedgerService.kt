...
class UnaddressedFindingsLedgerService(
  ...
  fun ledger(issueKey: IssueKey): UnaddressedFindingsLedger = database.read { unitOfWork ->
  ...
  fun verificationDispositions(issueKey: IssueKey): List<FeatureTaskRuntimeFindingVerificationDisposition> =
  ...
  fun repairLedgersByWorkflow(issueKey: IssueKey): Map<String, FeatureTaskRuntimeRepairLedger> =
    database.read { unitOfWork ->
      if (!unitOfWork.unaddressedFindings.issueExists(issueKey)) {
        throw UnaddressedFindingsLedgerAbsentError("No goal exists for issue key '$issueKey'.")
      }
  ...
  private fun isValidFinding(issueKey: IssueKey, finding: UnaddressedFinding): Boolean = finding.issueKey == issueKey &&
    finding.workflowId.value.isNotBlank() &&
    finding.subtaskId.value > 0 &&
