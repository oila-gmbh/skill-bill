...
object PortableReviewBaselineValidator {
  fun isSafePreImplementationRecovery(subtask: DecompositionSubtask): Boolean = !hasImplementationEvidence(subtask)
  ...
  fun validateStoredArtifact(request: PortableReviewBaselineValidationRequest): PortableReviewBaselineValidation {
    ...
    return when (val integrity = validateArtifactIntegrity(request)) {
      ...
      is PortableReviewBaselineValidation.Valid -> {
        when {
          hasImplementationEvidence(subtask) -> blocked(
          ...
          hasConflictingBranchEvidence(request, integrity.artifact) -> blocked(
            PortableReviewBaselineBlockedReason.BRANCH_EVIDENCE,
            "Goal subtask '${request.subtaskId}' has branch commits beyond the portable review baseline; " +
              "portable review state cannot be rehydrated safely.",
            integrity.artifact,
          )
          else -> integrity
        }
  ...
  private fun hasImplementationEvidence(subtask: DecompositionSubtask): Boolean = !subtask.commitSha.isNullOrBlank() ||
    subtask.lastResumableStep?.let { it !in PRE_IMPLEMENTATION_STEPS } == true

  private fun hasConflictingBranchEvidence(
    request: PortableReviewBaselineValidationRequest,
    artifact: PortableReviewBaseline,
  ): Boolean {
    val head = request.gitOperations.headCommitSha(request.repoRoot)
    return !head.ok || head.value.trim() != artifact.reviewBaseSha.trim()
  }
  ...
  private val PRE_IMPLEMENTATION_STEPS = setOf("create_branch", "preplan", "plan")
}
