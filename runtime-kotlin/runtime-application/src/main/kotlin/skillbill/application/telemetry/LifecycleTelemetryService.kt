...
class LifecycleTelemetryService(
  ...
  fun featureTaskRuntimeStarted(request: FeatureTaskRuntimeStartedRequest): Map<String, Any?> {
    val sessionId = SessionId(request.sessionId.value.ifBlank { generateLifecycleSessionId("ftr").value })
  ...
  fun prDescriptionGenerated(request: PrDescriptionGeneratedRequest): Map<String, Any?> {
    val sessionId = if (request.orchestrated) SessionId("") else generateLifecycleSessionId("prd")
    return when {
      request.orchestrated -> request.orchestratedPayload(telemetryLevelOrAnonymous(settingsProvider))
      else ->
        enabledStandaloneResult(settingsProvider, sessionId) { settings ->
