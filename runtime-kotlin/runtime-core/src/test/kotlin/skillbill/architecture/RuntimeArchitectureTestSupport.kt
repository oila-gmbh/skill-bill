...
internal const val mcpScaffoldRuntimePath =
  ...
  internal fun syntheticSourceFile(relativePath: String, source: String): SourceFile = SourceFile(
    ...
    imports = RuntimeArchitectureScanConstants.importPattern.findAll(source).map { it.groupValues[1].substringBefore(" as ") }.toList(),
    source = source,
  )
  ...
  internal fun installPortFunctionSignatures(sourceFile: SourceFile): List<InstallPortFunctionSignature> {
    ...
    return lines.mapIndexedNotNull { index, line ->
      val match = RuntimeArchitectureScanConstants.portFunctionStartPattern.find(line.trim()) ?: return@mapIndexedNotNull null
  ...
  internal fun assertInventoryMatchesAllowList(inventory: Skill522Inventory) {
    ...
    assertTrue(
      ...
      "SKILL-52.2 inventory must classify every entry in RuntimeArchitectureScanConstants.RAW_MAP_OPEN_BOUNDARY_ALLOWLIST " +
  ...
  internal fun advanceInventoryCursor(rawLines: List<String>, state: InventoryParseState): Int {
    ...
    val bulletMatch = if (heading == null) RuntimeArchitectureScanConstants.INVENTORY_BULLET_PATTERN.find(trimmed) else null
  ...
  internal fun buildInventoryEntry(
    ...
    val subtaskId = RuntimeArchitectureScanConstants.INVENTORY_SUBTASK_PATTERN.find(joinedTail)?.groupValues?.get(1)?.toIntOrNull()
...
internal fun findRawMapViolations(file: SourceFile): List<String> {
  ...
  lines.forEachIndexed { index, line ->
    ...
    val sigText = collectRawMapDeclarationSignature(lines, index, rawMapValDeclPattern.matches(trimmed))
