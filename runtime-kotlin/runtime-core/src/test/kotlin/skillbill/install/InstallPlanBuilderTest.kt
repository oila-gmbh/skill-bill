...
class InstallPlanBuilderTest {
  ...
  fun `plan includes base skills and dynamically selected platform pack skills`() {
    ...
    assertEquals(InstallPlanSkillKind.PLATFORM_PACK, skillsByName.getValue("bill-kotlin-quality-check").kind)
  ...
  fun `duplicate platform manifest slots pointing to one skill directory fail during planning`() {
    ...
    Files.writeString(
      manifest,
      Files.readString(manifest).replace(
        "declared_quality_check_file: \"quality-check/bill-collapsed-quality-check/content.md\"",
  ...
  private fun seedPlatformPack(
    repoRoot: Path,
    slug: String,
    qualityCheckName: String = "bill-$slug-quality-check",
