...
fun resolvedParentSpecPath(repoRoot: Path, parentSpecPath: Path): Path = if (parentSpecPath.isAbsolute) {
  parentSpecPath.normalize()
} else {
  repoRoot.resolve(parentSpecPath).normalize()
}
