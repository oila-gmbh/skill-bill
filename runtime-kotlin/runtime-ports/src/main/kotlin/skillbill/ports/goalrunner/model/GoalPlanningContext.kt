...
data class GoalPlanningContext(
  ...
  companion object {
    ...
    /**
     * The per-file read cap discovery and body resolution must both use. Reading one file under two
     * different limits lets the two passes parse different text, so the catalog can publish a heading
     * digest the resolver never reproduces and the selection is unresolvable forever.
     */
    ...
    /** Unresolved ids are model-authored text; the prompt echoes a bounded, single-line rendering. */
