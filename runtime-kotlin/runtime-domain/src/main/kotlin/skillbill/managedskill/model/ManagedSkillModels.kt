...
data class AgentSkillTargetId(val provider: String, val skillsPath: Path) {
  init {
    require(provider.isNotBlank()) { "Agent provider must not be blank." }
    require(skillsPath.isAbsolute) { "Agent target path must be absolute." }
  }

  val stableIdentity: String = "$provider:${skillsPath.normalize()}"
}
