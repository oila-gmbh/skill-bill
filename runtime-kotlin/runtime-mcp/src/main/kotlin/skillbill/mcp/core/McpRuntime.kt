...
object McpRuntime {
  ...
  fun triageFindings(
    ...
    if (!runtimeServices.telemetryService.isEnabled()) {
      return McpTriageSkippedContract(reason = "telemetry is disabled", reviewRunId = reviewRunId.value).toPayload()
    }
    if (orchestrated) {
      runtimeServices.reviewService.markOrchestrated(reviewRunId)
    }
  ...
  fun reviewStats(reviewRunId: ReviewRunId? = null, context: McpRuntimeContext = McpRuntimeContext()): Map<String, Any?> =
