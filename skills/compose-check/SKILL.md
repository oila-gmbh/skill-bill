...
- Keep parameter count reasonable. If you have more than ~7 parameters (excluding modifier and content), extract a state data class.
...
Every interactive or informational composable must be accessible:

- `Image` and `Icon`: provide `contentDescription` (string resource) or `null` if purely decorative.
- Decorative images: `contentDescription = null` (this tells TalkBack to skip them).
- Clickable surfaces: use `Modifier.clickable(onClickLabel = stringResource(...))` or `Modifier.semantics { }` for custom actions.
- Use `Modifier.semantics(mergeDescendants = true)` on container composables to group related content for screen readers.
- Use `Modifier.clearAndSetSemantics { }` when a parent already describes the content and children should not be announced individually.
- Ensure sufficient color contrast (WCAG AA minimum).
- Support dynamic font scaling — never set fixed text sizes that ignore accessibility settings.
- Test with TalkBack and Accessibility Scanner.

---

## 7. Composable Structure & Decomposition
...
- Stateless composables are easy to test — just pass state and assert UI.
- Use `composeTestRule.setContent { }` with controlled state.
- Assert with semantics: `onNodeWithText()`, `onNodeWithContentDescription()`, `onNodeWithTag()`.
- Test all UI states: loading, content, error, empty.
- Test interactions: clicks trigger the correct lambdas.
- Do NOT test ViewModel logic in Compose UI tests — that belongs in unit tests.
