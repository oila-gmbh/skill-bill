...
Follow the detailed step instructions in [content.md](content.md#step-1-collect-inputs).

The shell transition contract for this step is:

- Ask for the task spec and PR inputs in Step 1.
- After Step 2 is confirmed, call `feature_verify_started` and save the returned `session_id`.
- Open workflow state with `feature_verify_workflow_open`.
- Immediately call `feature_verify_workflow_update` to mark `collect_inputs` and `extract_criteria` completed, set `gather_diff` to running, and persist `input_context` plus `criteria_summary`.
...
Follow the detailed step instructions in [content.md](content.md#step-2-extract-acceptance-criteria).

The shell transition contract for this step is:

- Confirm the extracted criteria with the user before review work begins.
- Persist `criteria_summary` as the success artifact.
...
Follow the detailed step instructions in [content.md](content.md#step-3-gather-pr-diff).

The shell transition contract for this step is:

- Resolve the PR diff target from the user input.
- Persist `diff_summary` before advancing to the feature-flag audit.
- Update workflow state so `gather_diff` is completed and `feature_flag_audit` is running.
...
Follow the detailed step instructions in [content.md](content.md#step-4-feature-flag-audit-conditional).

Read [audit-rubrics.md](audit-rubrics.md) for the full feature flag audit rubric and output format.

The shell transition contract for this step is:

- The audit is legally skippable when the spec and diff do not require it.
- Persist `feature_flag_audit_result` either way.
- Update workflow state so `feature_flag_audit` is completed or skipped and `code_review` is running.
...
Follow the detailed step instructions in [content.md](content.md#step-5-code-review).
...
- Persist `review_result` after review finishes.
- Update workflow state so `code_review` is completed and `completeness_audit` is running.
...
Follow the detailed step instructions in [content.md](content.md#step-6-completeness-audit).

Read [audit-rubrics.md](audit-rubrics.md) for the completeness audit format and rules.

The shell transition contract for this step is:

- Persist `completeness_audit_result` when the audit succeeds.
- When the verify target changes materially during the same session, loop back to `gather_diff` or `code_review` and increment the next step's `attempt_count`.
- Otherwise, update workflow state so `completeness_audit` is completed and `verdict` is running.
...
Follow the detailed step instructions in [content.md](content.md#step-7-consolidated-verdict).

Read [audit-rubrics.md](audit-rubrics.md) for the verdict format and PR comment instructions.

The shell transition contract for this step is:

- Persist `verdict_result` after the verdict is delivered.
- Update workflow state so `verdict` is completed and `finish` is running.
