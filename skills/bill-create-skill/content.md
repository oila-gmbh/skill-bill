...
1. Intent-first intake. Never start by pasting a full taxonomy or asking the user to choose from every scaffold kind at once unless they explicitly ask for the raw payload format.
   ...
   Accept a real platform slug such as `java`, `kotlin`, `kmp`, or `cross-stack` when the user wants a horizontal skill with no owning platform.
...
6. Emit JSON payload. When the user accepts, build a payload shaped like:
   ...
   For orchestrator kinds (`horizontal`, `platform-override-piloted`, `platform-pack`), the scaffolder emits one provider-neutral source bundle for names listed in `subagent_specialists`. For `platform-pack`, omitting `subagent_specialists` defaults the bundle to the selected code-review specialist skills (`bill-<platform>-code-review-<area>`) with per-area descriptions, not generic TODO copies. Source stubs land in `<orchestrator-skill-dir>/native-agents/agents.yaml`; generated Claude, Codex, OpenCode, and Junie artifacts are rendered into the install cache when users install the skill. Render and install output also generate the `Subagent Spawn Runtime Notes` wrapper section from the native-agent source bundle instead of storing runtime wiring in `content.md`. Set `no_subagents: true` to opt out explicitly when an orchestrator should not delegate to native subagents (the two flags cannot both be set together). Specialist names must match `^[a-z][a-z0-9-]*$`, must be unique, and are NOT valid for leaf kinds such as `code-review-area` or `add-on`. After scaffolding, fill in TODO placeholders in custom `subagent_specialists` stubs, then run `skill-bill render` to validate sources before shipping.
