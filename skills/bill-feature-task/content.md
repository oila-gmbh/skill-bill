...
Also parse exactly one optional `code-review:auto|inline|delegated` token. Omission resolves to `inline`; `delegated` is the experimental full-depth tier and is reached only by an explicit token. Malformed, unknown, duplicate, or conflicting values fail before confirmation, workflow opening, or delegation. Forward the resolved selection unchanged to either sidecar.
