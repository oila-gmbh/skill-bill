...
description: Use when generating a PR title, description, and QA steps from the current branch changes. Works standalone or as part of bill-feature-task. Use when user mentions write PR description, generate PR, PR title, or create pull request.
---
...
Orchestrated invocation (when called from `bill-feature-task` or similar parent that passes `orchestrated=true`): after creating or opening the PR, compare the normalized generated body with the actual PR body passed to `gh pr create` or read back with `gh pr view --json body` when available. Call `pr_description_generated` with `orchestrated=true`, `was_edited_by_user=true` when those bodies differ, and the same fields. The tool returns `{"mode": "orchestrated", "telemetry_payload": {...}}`. Return that payload to the orchestrator — it will embed it in its own finished event.
