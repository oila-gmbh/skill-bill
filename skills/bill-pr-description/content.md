...
Orchestrated invocation (when called from a parent that passes `orchestrated=true`): after creating or opening the PR, compare the normalized generated body with the actual PR body passed to `gh pr create` or read back with `gh pr view --json body` when available. Call `pr_description_generated` with `orchestrated=true`, `was_edited_by_user=true` when those bodies differ, and the same fields. The tool returns `{"mode": "orchestrated", "telemetry_payload": {...}}`. Return that payload to the orchestrator — it will embed it in its own finished event.
