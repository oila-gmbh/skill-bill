...
`parallel_review_agent`, immutable `review_base_sha`, baseline untracked
inventory, completed and reserved review-pass state, and cap disposition) and
runs the full phase loop for that subtask by calling
...
second full lane with the same execution mode and exact delta, but never pass a
parallel argument into either lane recursively. Preserve the selected mode
through repair and audit re-entry. Reserve before launch, reuse a previously
reserved pass after a crash instead of counting another, count coordinated
lanes as one pass, and never run a third review. At a two-pass unresolved
Blocker/Major cap, preserve full evidence, return the non-approval
`review_cap_reached` disposition and compact path-free summary to the parent,
then continue with the normal audit and later gates.
