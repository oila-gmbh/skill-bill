...
`subtask_id`, `workflow_id`, `spec_path`, `code_review_mode`, `review_base_sha`,
baseline untracked inventory, review pass count, and cap disposition) and runs the full phase loop for
...
earlier-subtask commits. Preserve the selected mode through repair and audit
re-entry. Reserve before launch, count parallel lanes as one pass, and never
run a third review. At a two-pass unresolved Blocker/Major cap, preserve full
evidence, return the non-approval `review_cap_reached` disposition to the
parent, and continue with the normal audit and later gates.
