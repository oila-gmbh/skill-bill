...
The ledger is the curated carry-forward, not the full prior context. Each entry
pairs a resolved finding with the constructs holding it closed and an explicit
status, so a later round can be told it is disturbing settled work.
...
1. Receipts accumulate into a durable remediation repair ledger held under the
   goal-subtask review state, ordered by round, surviving process death, parent
   resume, and cross-run continuation without duplication or loss.
2. Every entry carries an explicit status of at least resolved, superseded, or
   reopened, together with the round that produced it and the round that last
   changed its status.
