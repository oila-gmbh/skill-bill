# SKILL-137 Subtask 7 - Compatibility, Telemetry, and End-to-End Hardening
...
10. Prompt byte-budget tests cover ASCII, multibyte UTF-8, maximum item counts, oversized required fields, compact references, and add-on content. Required JSON is never truncated.
11. Persistence round-trip and upgrade tests prove private evidence/delivered projections remain separate, and startup behavior against incompatible records is deterministic.
12. Standalone feature-task, decomposed goal child, multi-subtask isolation, runtime mode, prose mode, feature verification, retry, continuation, and terminal success/block/failure paths pass.
13. `runtime-kotlin/ARCHITECTURE.md` and relevant docs describe the least-context principle, projection matrix, receipt-as-claim rule, repository authority, checkpoint freshness, private evidence, and telemetry separation.
14. Governed `content.md` for feature routing/task/runtime/prose/goal/subtask-runner, code review where needed, PR description, code check, and feature verification uses the new contract names and does not reintroduce whole-artifact instructions.
15. Generated wrappers, support pointers, provider-specific agent outputs, installed staging, prompts, diff bodies, and raw tool output remain uncommitted.
16. Repository validation completes with:
...
17. Because governed skill source changes require install refresh, final handoff tells the operator to run `./install.sh` after the goal; no installer or uninstall flow runs inside goal continuation.
...
This is the integration and hardening subtask; it runs only after all runtime, prose, finalization, and verification projections exist.
...
- Standalone, goal-child, prose, runtime, verify, retry, resume, and end-to-end suites.
- Full repository validation commands from acceptance criterion 16.
...
.feature-specs/SKILL-137-phase-context-minimization/spec_subtask_7_compatibility-telemetry-and-end-to-end-hardening.md
