...
8. Outside `.feature-specs/` and `agent/history.md`, no live source, contract, doc, or script references `bill-feature-task-runtime`.
9. Outside `.feature-specs/` and `agent/history.md`, the only surviving references to `bill-feature-task` are the durable workflow identity (DB CHECK constraint, `workflow-state-schema.yaml` `const`, telemetry enum, `FeatureTaskRuntimePhaseWorkflowDefinition`) and the `skill-bill feature-task` CLI command name.
...
and 9 with a repository sweep for both names, excluding `.feature-specs/` and `agent/history.md`,
